...
export function LifeEventSheet({
  ...
  open: boolean;
  onClose: () => void;
  onSubmit: (values: LifeEventInput) => void;
}) {
  ...
  return (
    <Sheet
      ...
      description="Guarda hechos reales que cambian tu trayectoria o contexto dentro de la timeline."
    >
      <form
        className="space-y-4"
      ...
      >
        <label htmlFor={`${idPrefix}-title`} className="space-y-2 text-sm">
          ...
          <Input id={`${idPrefix}-title`} placeholder="Ascenso, certificación, nuevo cliente..." {...form.register("title")} />
        </label>
        <label htmlFor={`${idPrefix}-description`} className="space-y-2 text-sm">
          <span className="text-muted-foreground">Descripción</span>
          <Textarea id={`${idPrefix}-description`} placeholder="Qué pasó y por qué importa." {...form.register("description")} />
        </label>
        ...
        <label htmlFor={`${idPrefix}-tags`} className="space-y-2 text-sm">
          <span className="text-muted-foreground">Tags</span>
          <Input id={`${idPrefix}-tags`} placeholder="salary, cert, project" {...form.register("tags")} />
        </label>
        <label htmlFor={`${idPrefix}-reflection`} className="space-y-2 text-sm">
          <span className="text-muted-foreground">Reflexión</span>
          <Textarea id={`${idPrefix}-reflection`} placeholder="Por qué este evento cambia tu contexto." {...form.register("reflection")} />
        </label>
